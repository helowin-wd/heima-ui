<template>
  <div class="index">
    <button
      class="one-button"
      :class="[`one-button-${type}`,{'is-plain':plain,'is-round': round,'is-circle': circle,'is-disabled':disabled}]"
      @click="handleClick"
      :disabled="disabled"
    >
      <!-- 如果没传入icon图标，则不显示i标签 -->
      <i v-if="icon" :class="`vh-icon-${icon}`"></i>
